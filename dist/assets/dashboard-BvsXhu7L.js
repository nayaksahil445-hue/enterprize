import{a as e,c as t,d as n,i as r,n as i,o as a,p as o,r as s}from"./utils-B-dAw4_9.js";/* empty css              */t()||(window.location.href=`/auth.html`);var c=a();o(),document.getElementById(`dash-avatar`).textContent=(c?.name||`U`)[0].toUpperCase(),document.getElementById(`dash-name`).textContent=`Welcome, ${c?.name||`User`}`,document.getElementById(`dash-email`).textContent=c?.email||``,document.querySelectorAll(`.dash-tab`).forEach(e=>{e.addEventListener(`click`,()=>{document.querySelectorAll(`.dash-tab`).forEach(e=>e.classList.remove(`active`)),document.querySelectorAll(`.dash-pane`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),document.getElementById(`pane-${e.dataset.pane}`).classList.add(`active`)})});var l=window.location.hash.replace(`#`,``);if(l){let e=document.querySelector(`.dash-tab[data-pane="${l}"]`);e&&e.click()}async function u(){try{let t=await i(`/orders/my`),n=document.getElementById(`orders-content`);if(!t||!t.length){n.innerHTML=`
