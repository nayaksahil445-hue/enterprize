import{a as e,c as t,d as n,f as r,n as i,p as a}from"./utils-B-dAw4_9.js";/* empty css              */var o=`http://localhost:5000/api`,s=new URLSearchParams(window.location.search).get(`id`),c=null,l=1;a(),_(),s?u():d();async function u(){try{let e=localStorage.getItem(`je_token`),t=e?{Authorization:`Bearer ${e}`}:{},n=await fetch(`${o}/products/${s}`,{headers:t});if(!n.ok)throw Error();c=await n.json(),f(c),m(),g()}catch{d()}}function d(){document.getElementById(`product-loading`).style.display=`none`,document.getElementById(`product-error`).style.display=`block`,document.getElementById(`recommended-section`).style.display=`none`}function f(t){document.title=`${t.name} | Jagannath Enterprises`,document.getElementById(`product-loading`).style.display=`none`;let n=document.getElementById(`product-content`);n.style.display=`block`;let i=t.originalPrice&&t.originalPrice>t.price?Math.round((t.originalPrice-t.price)/t.originalPrice*100):0,a=t.stock>0,o=t.stock>0&&t.stock<=(t.lowStockThreshold||10),s=``;if(t.specifications){let e=t.specifications instanceof Map?Object.fromEntries(t.specifications):t.specifications;s=Object.entries(e).map(([e,t])=>`<div class="pd-spec-row"><span class="pd-spec-key">${e}</span><span class="pd-spec-val">${t}</span></div>`).join(``)}n.innerHTML=`
    <div class="pd-grid">
      <div class="pd-image-wrap">
        <div class="pd-badge-wrap">
          <span class="pd-badge">${t.category}</span>
          ${i>0?`<span class="pd-badge pd-badge-discount">${i}% OFF</span>`:``}
          ${a?``:`<span class="pd-badge pd-badge-oos">Out of Stock</span>`}
        </div>
      ...
      <div class="pd-info">
        ...
        <div class="pd-price-row">
          ...
          ${i>0?`<span class="pd-discount-tag">Save ${i}%</span>`:``}
        </div>

        <p class="pd-desc">${t.description||`No description available.`}</p>

        <div class="pd-stock">
          <span class="pd-stock-dot ${a?o?`pd-stock-low`:`pd-stock-in`:`pd-stock-out`}"></span>
          ${a?o?`<span style="color:#f59e0b;">Only ${t.stock} left — order soon!</span>`:`<span style="color:#22c55e;">In Stock (${t.stock} available)</span>`:`<span style="color:#ef4444;">Currently Out of Stock</span>`}
        </div>
  ...
  `}async function g(){try{let t=localStorage.getItem(`je_token`),n=t?{Authorization:`Bearer ${t}`}:{},i=(await(await fetch(`${o}/products/recommendations`,{headers:n})).json()).filter(e=>e._id!==s).slice(0,4),a=document.getElementById(`recommended-grid`);if(!i.length){document.getElementById(`recommended-section`).style.display=`none`;return}a.innerHTML=i.map(t=>`
    ...
    `).join(``)}catch{document.getElementById(`recommended-section`).style.display=`none`}}function _(){let e=JSON.parse(localStorage.getItem(`je_cart`)||`[]`).reduce((e,t)=>e+(t.qty||1),0),t=document.getElementById(`cart-count`);t&&(t.textContent=e,t.style.display=e>0?`flex`:`none`)}