...
Hours: 10:00 AM - 7:00 PM (Mon-Sat)`);let t=document.getElementById(`je-chat-messages`),n=document.createElement(`div`);n.className=`je-chat-card`,n.innerHTML=`
      <button class="je-chat-btn je-chat-btn-secondary" id="je-chat-back-btn">Main Menu</button>
    `,n.querySelector(`#je-chat-back-btn`)?.addEventListener(`click`,()=>{n.remove(),this.startConversation()}),t?.appendChild(n),this.scrollToBottom()}async handleCancelOrderOption(){if(!o()){this.addBotMessage(`Please log in to your account first to view and cancel your orders.`),this.showQuickOptions([{text:`Sign In Now`,action:()=>window.location.href=`/auth`},{text:`Main Menu`,action:()=>this.startConversation()}]);return}this.addBotMessage(`Fetching your active orders. Please wait...`),this.state=e.SELECT_ORDER;try{if(this.activeOrders=(await u(`/orders/my`)||[]).filter(e=>e.orderStatus===`Placed`||e.orderStatus===`Confirmed`),this.activeOrders.length===0){this.addBotMessage(`You do not have any active orders that can be cancelled. Shipped or Delivered orders cannot be cancelled via chat.`),this.showQuickOptions([{text:`Main Menu`,action:()=>this.startConversation()}]);return}this.addBotMessage(`Which order would you like to cancel?`);let e=document.getElementById(`je-chat-messages`);this.activeOrders.forEach(t=>{let n=document.createElement(`div`);n.className=`je-chat-card`,n.style.cursor=`pointer`,n.style.transition=`var(--transition)`,n.innerHTML=`
          <div style="font-weight:700; font-size:0.82rem; color:var(--gold);">Order #${t.orderNumber}</div>
          <div style="font-size:0.75rem; color:var(--text-muted); margin-top:0.15rem;">Placed: ${g(t.createdAt)}</div>
          <div style="font-size:0.78rem; font-weight:700; color:#fff; margin-top:0.3rem;">Total: ${m(t.totalAmount)}</div>
    ...
    `,n.querySelector(`#je-cancel-back`)?.addEventListener(`click`,()=>{n.remove(),this.addUserMessage(`Go Back`),this.askCancellationReason()}),n.querySelector(`#je-cancel-confirm`)?.addEventListener(`click`,()=>{n.remove(),this.addUserMessage(`Confirm Cancellation`),this.executeCancellation()}),t?.appendChild(n),this.scrollToBottom()}async executeCancellation(){this.state=e.CANCELLING,this.addBotMessage(`Processing your cancellation request. Please hold on...`);try{let t={reason:this.cancellationReason===`Other`?`Other`:this.cancellationReason,customReason:this.customReason,feedback:this.additionalFeedback};if(await u(`/orders/${this.selectedOrder._id}/cancel`,{method:`PATCH`,body:JSON.stringify(t)})){this.state=e.SUCCESS,this.addBotMessage(`Your order has been cancelled successfully. We appreciate your feedback.`);let n={orderId:this.selectedOrder._id,orderNumber:this.selectedOrder.orderNumber,userId:this.selectedOrder.user,cancelledAt:new Date().toISOString(),reason:t.reason,customReason:t.customReason,feedback:t.feedback,orderStatus:`Cancelled`,items:this.selectedOrder.items};if(localStorage.setItem(`je_cancel_record_${this.selectedOrder._id}`,JSON.stringify(n)),window.location.pathname.includes(`dashboard`)){let e=document.querySelector(`[data-pane="orders"]`);e&&e.click()}window.location.pathname.includes(`tracking`)&&window.location.reload();let r=document.getElementById(`je-chat-messages`),i=document.createElement(`div`);i.className=`je-chat-card`,i.innerHTML=`
        ...
        `,i.querySelector(`#je-reorder-btn`)?.addEventListener(`click`,()=>{this.reorderItems()}),i.querySelector(`#je-contact-support-btn`)?.addEventListener(`click`,()=>{i.remove(),this.handleContactSupportOption()}),r?.appendChild(i),this.scrollToBottom()}}catch(e){console.error(e),this.addBotMessage(`Cancellation failed: ${e.message||`Server error`}`),this.showQuickOptions([{text:`Retry Cancellation`,action:()=>this.showConfirmationSummary()},{text:`Main Menu`,action:()=>this.startConversation()}])}}async reorderItems(){this.addBotMessage(`Adding items from the cancelled order back to your cart...`);try{for(let e of this.selectedOrder.items)await u(`/cart/add`,{method:`POST`,body:JSON.stringify({productId:e.product,qty:e.qty})});p(`Items added to cart!`,`success`),this.addBotMessage(`All items have been added to your cart. Would you like to proceed to checkout?`),this.showQuickOptions([{text:`Checkout Now`,action:()=>window.location.href=`/checkout`},{text:`View Cart Drawer`,action:()=>{window.location.href=`/?open-cart=true`}},{text:`Main Menu`,action:()=>this.startConversation()}])}catch{p(`Failed to reorder items`,`error`),this.addBotMessage(`Some items could not be added to your cart. They might be out of stock.`),this.showQuickOptions([{text:`Main Menu`,action:()=>this.startConversation()}])}}};window.addEventListener(`DOMContentLoaded`,()=>{window.location.pathname.includes(`admin`)||(window.jeChatbot=new t)});function n(){if(typeof window<`u`){let e=window.location.hostname;if(e===`localhost`||e===`127.0.0.1`)return`http://localhost:5000/api`;if(/^192\.168\.|^10\.|^172\.(1[6-9]|2[0-9]|3[01])\./.test(e))return`http://${e}:5000/api`}return`https://jaganath-backend.onrender.com/api`}var r=n();function i(){return localStorage.getItem(`je_token`)}function a(){let e=localStorage.getItem(`je_user`);return e?JSON.parse(e):null}function o(){return!!i()}function s(){let e=a();return e&&e.role===`admin`}function c(e,t){localStorage.setItem(`je_token`,e),localStorage.setItem(`je_user`,JSON.stringify(t))}function l(){localStorage.removeItem(`je_token`),localStorage.removeItem(`je_user`),localStorage.removeItem(`je_cart`),window.location.href=`/`}async function u(e,t={}){let n=i(),a={headers:{"Content-Type":`application/json`,...n?{Authorization:`Bearer ${n}`}:{},...t.headers},...t};delete a.headers;let o={...t,headers:{"Content-Type":`application/json`,...n?{Authorization:`Bearer ${n}`}:{},...t.headers||{}}};try{let t=await fetch(`${r}${e}`,o),n=await t.json();if(!t.ok){if(t.status===401)return l(),null;throw Error(n.message||`Request failed`)}return n}catch(t){throw console.error(`API Error [${e}]:`,t),t}}var d=null;function f(){return d||(d=document.createElement(`div`),d.id=`toast-container`,d.style.cssText=`
      position: fixed; top: 1.5rem; right: 1.5rem; z-index: 99999;
      display: flex; flex-direction: column; gap: 0.75rem;
      pointer-events: none; max-width: 420px;
    `,document.body.appendChild(d)),d}function p(e,t=`success`,n=3500){let r=f(),i=document.createElement(`div`),a={success:`✓`,error:`✕`,warning:`⚠`,info:`ℹ`},o={success:`linear-gradient(135deg, #16a34a, #22c55e)`,error:`linear-gradient(135deg, #dc2626, #ef4444)`,warning:`linear-gradient(135deg, #d97706, #f59e0b)`,info:`linear-gradient(135deg, #2563eb, #3b82f6)`};i.style.cssText=`
  ...
  `,document.head.appendChild(e)}function m(e){return`₹`+Number(e||0).toLocaleString(`en-IN`)}function h(e){return new Date(e).toLocaleString(`en-IN`,{day:`2-digit`,month:`short`,year:`numeric`,hour:`2-digit`,minute:`2-digit`})}function g(e){return new Date(e).toLocaleDateString(`en-IN`,{day:`2-digit`,month:`short`,year:`numeric`})}function _(){let e=document.querySelector(`.nav-right`);if(!e)return;let t=a(),n=document.getElementById(`nav-auth-area`);n&&n.remove();let r=document.createElement(`div`);r.id=`nav-auth-area`,r.style.cssText=`display:flex; align-items:center; gap:0.8rem;`,t?r.innerHTML=`
      <div class="nav-user-menu" style="position:relative;">
        ...
        ">
          ...
          ${t.role===`admin`?`<a href="/admin" style="display:flex;align-items:center;gap:0.6rem;padding:0.75rem 1.25rem;color:var(--text-muted);font-size:0.82rem;font-weight:500;transition:all 0.2s;" onmouseover="this.style.color='var(--gold)';this.style.background='rgba(201,162,39,0.05)'" onmouseout="this.style.color='var(--text-muted)';this.style.background='none'">⚙️ Admin Panel</a>`:``}
    ...
    `;let i=e.querySelector(`.hamburger`);i?e.insertBefore(r,i):e.appendChild(r);let o=document.getElementById(`nav-user-btn`),s=document.getElementById(`nav-user-dropdown`);o&&s&&(o.addEventListener(`click`,e=>{e.stopPropagation(),s.style.display=s.style.display===`none`?`block`:`none`}),document.addEventListener(`click`,()=>{s.style.display=`none`}));let c=document.getElementById(`nav-logout-btn`);c&&c.addEventListener(`click`,l);let u=document.getElementById(`hamburger`),d=document.querySelector(`.nav-links`);if(u&&d){let e=document.getElementById(`nav-overlay`);e||(e=document.createElement(`div`),e.id=`nav-overlay`,e.className=`nav-overlay`,document.body.appendChild(e));let t=t=>{t?.stopPropagation();let n=d.classList.toggle(`open`);u.classList.toggle(`active`),e.classList.toggle(`open`),document.getElementById(`navbar`)?.classList.toggle(`menu-open`,n),document.body.style.overflow=n?`hidden`:``},n=()=>{d.classList.remove(`open`),u.classList.remove(`active`),e.classList.remove(`open`),document.getElementById(`navbar`)?.classList.remove(`menu-open`),document.body.style.overflow=``},r=u.cloneNode(!0);u.parentNode.replaceChild(r,u),r.addEventListener(`click`,t),e.addEventListener(`click`,n),d.querySelectorAll(`.nav-link`).forEach(e=>{e.addEventListener(`click`,n)});let i=d.querySelector(`.nav-back-btn`)||document.getElementById(`nav-back-btn`);i&&i.addEventListener(`click`,n)}}function v(e,t=5){let n=``;for(let r=1;r<=t;r++)r<=Math.floor(e)||r-.5<=e?n+=`<span style="color:var(--gold);">★</span>`:n+=`<span style="color:var(--text-dim);">★</span>`;return n}function y(e,t){let n=(t||``).toLowerCase(),r=(e||``).toLowerCase(),i=`industrial steel cupboard manufacturer - steel almirah price`;return n.includes(`storage`)||r.includes(`cupboard`)||r.includes(`almirah`)?i=`steel almirah with locker 2 door 3 door steel almirah - industrial steel cupboard manufacturer New Delhi Odisha`:n.includes(`luxury`)||r.includes(`dressing`)?i=`steel wardrobe online India - steel dressing table manufacturer`:n.includes(`office`)||r.includes(`cabinet`)?i=`office almirah manufacturer - steel cupboard India steel file cabinet`:n.includes(`shelving`)||r.includes(`shelf`)?i=`heavy duty industrial steel cupboard manufacturer near me New Delhi Odisha`:(n.includes(`vintage`)||r.includes(`chest`)||r.includes(`drawer`))&&(i=`4 door steel almirah chest of drawers - best steel almirah price`),`${e} (${i})`}export{m as a,s as c,c as d,p as f,g as i,o as l,_ as m,u as n,y as o,v as p,h as r,a as s,r as t,l as u};